...
unsigned char *md5 (unsigned char *M, md5_size len, unsigned char *_digest)
{
	...
	ctx->buf = malloc (buflen);
	ctx->size = 0;
	ctx->bits = 0;

	/* Init registries */
	ctx->regs.A = 0x67452301;
	ctx->regs.B = 0xefcdab89;
	ctx->regs.C = 0x98badcfe;
	ctx->regs.D = 0x10325476;

	do {
		memcopy (ctx->buf + ctx->size, M + ctx->bits, buflen - ctx->size);
		ctx->size += buflen - ctx->size;
		md5_update (ctx);
	} while (len - ctx->bits > 64);

	md5_final (_digest, ctx);
...
{
	ctx->buf = malloc (MD5_BUFFER);
	memset (ctx->buf, '\0', MD5_BUFFER);
	ctx->size = 0;
	ctx->bits = 0;
...
/*
 * Update a ctx by concatenating a new block
 */
void md5_update (struct md5_ctx *ctx)
{
	...
	for (i = 0; ctx->size - i > 63; i += 64) {
		memcopy (buffer, ctx->buf + i, 64);
		md5_encode (buffer, ctx);
		ctx->bits += 64;
	}
	memcopy (buffer, ctx->buf + i, ctx->size - i);
	memcopy (ctx->buf, buffer, ctx->size - i);
	ctx->size -= i;
}

void md5_final (unsigned char *digest, struct md5_ctx *ctx)
{
	unsigned char buffer [64]; /* 512 bits */
	int i;
	...
	if (ctx->size + 1 > 56) { /* We have to create another block */
		memcopy (buffer, ctx->buf, ctx->size);
		memcopy (buffer + ctx->size, MD5_PADDING, 64 - ctx->size);
		md5_encode (buffer, ctx);
		ctx->bits += ctx->size;
		ctx->size = 0;
		/* Proceed final block */
		...
		md5_addsize (buffer, 56, ctx->bits);
		md5_encode (buffer, ctx);
	} else {
		memcopy (buffer, ctx->buf, ctx->size);
		ctx->bits += ctx->size;

		memcopy (buffer + ctx->size, MD5_PADDING, 56 - ctx->size);
		md5_addsize (buffer, 56, ctx->bits);
		md5_encode (buffer, ctx);
	}
	/* update digest */
	for (i = 0; i < 4; i++)
		digest [i] = (unsigned char) ((ctx->regs.A >> (i*8)) & 0xFF);
	for (; i < 8; i++)
		digest [i] = (unsigned char) ((ctx->regs.B >> ((i-4)*8)) & 0xFF);
	for (; i < 12; i++)
		digest [i] = (unsigned char) ((ctx->regs.C >> ((i-8)*8)) & 0xFF);
	for (; i < 16; i++)
		digest [i] = (unsigned char) ((ctx->regs.D >> ((i-12)*8)) & 0xFF);
}

static void md5_encode (unsigned char *buffer, struct md5_ctx *ctx)
{
	unsigned int a = ctx->regs.A, b = ctx->regs.B, c = ctx->regs.C, d = ctx->regs.D;
	unsigned int x[16];
